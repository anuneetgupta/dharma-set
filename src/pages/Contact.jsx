...
import {
  Mail, MessageCircle, MapPin, Send, CheckCircle, AlertCircle,
  MessageSquare, Clock, ChevronDown, ChevronUp, Inbox
} from 'lucide-react';
...
const STATUS_CONFIG = {
  unread:  { label: 'Awaiting Reply', color: 'text-amber-400',   bg: 'bg-amber-500/10',   border: 'border-amber-500/30',   dot: 'bg-amber-400'   },
  read:    { label: 'Under Review',   color: 'text-indigo-400',  bg: 'bg-indigo-500/10',  border: 'border-indigo-500/30',  dot: 'bg-indigo-400'  },
...
// ── My Replies sub-component ─────────────────────────────────────────────────
function MyMessages({ authFetch }) {
  const [messages, setMessages] = useState([]);
  const [loading, setLoading]   = useState(true);
  const [expanded, setExpanded] = useState(null);

  useEffect(() => {
    authFetch('/contact/my-messages')
      .then(r => r.json())
  ...
  if (messages.length === 0) {
    return (
      <div className="text-center py-10 text-white/30">
        ...
        <p className="text-sm">No messages yet. Send us a message using the form!</p>
      </div>
    );
  }
          ...
          >
            ...
            >
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${cfg.bg} ${cfg.border} ${cfg.color}`}>
                    {cfg.label}
                  </span>
                ...
                <p className="text-sm font-medium text-white/80 truncate">{msg.subject || 'General Inquiry'}</p>
            ...
            <AnimatePresence>
              {isOpen && (
                ...
                >
                  <div className="px-4 pb-5 flex flex-col gap-3 border-t border-white/[0.05] pt-3">
                    {/* Original message */}
                    <div className="bg-white/[0.03] rounded-xl p-4">
                      <p className="text-xs text-white/30 uppercase tracking-wider font-medium mb-2 flex items-center gap-1.5">
                    ...
                    {hasReply ? (
                      <div className="bg-emerald-500/8 border border-emerald-500/25 rounded-xl p-4">
                        <p className="text-xs text-emerald-400/70 uppercase tracking-wider font-medium mb-2 flex items-center gap-1.5">
                          <CheckCircle size={11} /> Reply from Dharma Setu Team
                          {msg.repliedAt && (
                            <span className="ml-auto text-white/25 normal-case font-normal">
                              {new Date(msg.repliedAt).toLocaleDateString('en-IN', {
                                day: '2-digit', month: 'short', year: 'numeric',
                              })}
                            </span>
                          )}
                        </p>
                        <p className="text-sm text-white/75 leading-relaxed whitespace-pre-wrap">{msg.adminReply}</p>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2 text-xs text-amber-400/60 px-1">
                        <Clock size={12} />
                        <span>We'll reply soon — usually within 24 hours. 🙏</span>
                      </div>
                    )}
                  </div>
                </motion.div>
...
export default function Contact() {
  ...
  const [activeTab, setActiveTab] = useState('send'); // 'send' | 'replies'
  ...
  const handleSubmit = async e => {
    ...
    try {
      // Pass JWT if user is logged in so server can save userId
      ...
      const res  = await fetch(`${API_BASE}/contact`, { method: 'POST', headers, body: JSON.stringify(form) });
      const data = await res.json();

      if (data.success) {
        setStatus('success');
        setForm(f => ({ name: user?.name || '', email: user?.email || '', subject: '', message: '' }));
        // Auto-switch to replies tab so user can track their message
        if (user) setTimeout(() => setActiveTab('replies'), 1600);
      } else {
        ...
        <div className="page-container">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 max-w-6xl mx-auto">
            ...
            <div className="lg:col-span-2 flex flex-col gap-4">
              <motion.h2 variants={fade} initial="hidden" whileInView="visible" viewport={{ once: true }} className="font-serif text-xl text-white/80 mb-2">
                Reach Us
              </motion.h2>
                  ...
                  <div className="flex border-b border-white/[0.06]">
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-2 py-3.5 text-sm font-medium transition-all duration-200 ${
                        activeTab === 'send'
                          ? 'text-gold-400 border-b-2 border-gold-500/60 bg-gold-500/5'
                    ...
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-2 py-3.5 text-sm font-medium transition-all duration-200 ${
                        activeTab === 'replies'
                          ? 'text-emerald-400 border-b-2 border-emerald-500/60 bg-emerald-500/5'
                          : 'text-white/40 hover:text-white/70'
                      }`}
                    >
                      <MessageSquare size={14} /> My Replies
                    </button>
                  </div>
                )}
                ...
                <div className="p-8 sm:p-10">
                  <AnimatePresence mode="wait">
                    ...
                    {user && activeTab === 'replies' ? (
                      ...
                      >
                        <h2 className="font-serif text-2xl text-emerald-400 mb-1">My Messages</h2>
                        <p className="text-sm text-white/40 mb-6">Track your messages and see replies from our team.</p>
                        <MyMessages authFetch={authFetch} />
                      </motion.div>
                      ...
                      >
                        ...
                        <p className="text-sm text-white/40 mb-8">
                          ...
                          {user && <span className="text-indigo-400/70"> Replies appear in the "My Replies" tab.</span>}
                        </p>

                        {status === 'success' && (
                          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
                            className="flex items-center gap-3 px-4 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm mb-6">
                            <CheckCircle size={16} />
                            {user
                              ? 'Message sent! Check the "My Replies" tab for our response. 🙏'
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                          ...
                          {!user && (
                            <p className="text-center text-xs text-white/25">
                              💡 <a href="/login" className="text-indigo-400/70 hover:text-indigo-400 transition-colors underline underline-offset-2">Log in</a> to track admin replies directly in this page.
                            </p>
                          )}
                        </form>
                      </motion.div>
